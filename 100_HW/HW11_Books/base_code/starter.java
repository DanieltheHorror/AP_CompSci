...
class starter {
		...
		*/
		while(true){
			...
			if (choice.equals("file")) {
				...
				System.out.println("The word '" + check + "' is in the text " + checksArray(arr, check) + " times");
			} else if (choice.equals("common")) {
				System.out.println("The most common word is '" + mostCommon(arr) + "'. It is used " + checksArray(arr, mostCommon(arr)) + " times.");
			} else if (choice.equals("change")) {
				System.out.println("What is the new file you would like?");
				String newChoice = sc.nextLine();
				arr = fillArray(newChoice);
			}
	...
	*/
	public static void printsNumber(String[] arr){
		Scanner sc = new Scanner(System.in);
	...
	*/
	public static String mostCommon(String[] arr){
		...
		String word = "";
		for (int i = 0; i < arr.length; i++) {
			if (checksArray(arr, arr[i]) > max) {
				max = checksArray(arr,arr[i]);
				word = arr[i];
			}
		}
